...
class UserAdmin(BaseUserAdmin):
    list_display = ('username', 'email', 'role', 'is_verified', 'is_approved', 'is_staff')
...
#         count = teachers.update(is_approved=True)
#         for teacher in teachers:
#             send_teacher_approval_email_task.delay(teacher.id, approved=True)
...
#             teacher.save()
#             send_teacher_approval_email_task.delay(teacher.id, approved=False)
#             count += 1
#         self.message_user(request, f"{count} teacher(s) rejected successfully.")
