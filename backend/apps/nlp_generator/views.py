...
#         if not paragraph or not topic_ids or not difficulty or not subject_id:
#             if request.accepted_renderer.format == 'html':
...
#                 )
#             return Response(
#                 {"error": "All fields (paragraph, topics, difficulty) are required."},
#                 status=status.HTTP_400_BAD_REQUEST
#             )
...
#                 messages.success(request, f"{len(saved_questions)} questions were generated and saved successfully.")
#             if errors:
#                 messages.warning(request, f"{len(errors)} questions could not be saved due to validation errors.")
#             return redirect("generate_mcq")
