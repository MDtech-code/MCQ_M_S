...
#         candidate_tokens = [token for token in sent if token.pos_ in ["NOUN", "PROPN"] and not token.is_stop]
...
#         if question_text == sentence or len(question_text.split()) < 5:
#             continue
...
#         # Fallback to ensure we always have 3 distractors
#         while len(distractors) < 3:
#             fallback = random.choice(["Force", "Inertia", "Momentum", "Velocity"])
#             if fallback != answer and fallback not in distractors:
#                 distractors.append(fallback)
